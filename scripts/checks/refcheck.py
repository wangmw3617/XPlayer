...
def main():
    ...
    if maybe_missing:
        # 这些只是启发式提示，不作为失败依据。
        # 经验值（2026-09-19 用本地真编译器校准过）：这一类的**误报率极高** ——
        # 当时报出 7 条，`assembleDebug` 全绿，一条都没中。常见成因：
        #   1. 名字被截断：报 `Name` 其实在指 `localName` 的结尾片段；
        #   2. KDoc 代码示例、字符串模板里的文本被当成代码；
        #   3. 同名局部变量 / 参数被当成类型名。
        # 所以：**有本地构建能力时直接跑 assembleDebug，别信这段**；
        # 只有在拿不到编译器（比如纯离线改代码）时才拿它当线索，
        # 而且必须核对完整标识符，不能只看它报的名字。
        print(f"  提示：{len(maybe_missing)} 处疑似缺少导入（启发式，误报率高，请核对完整标识符）")
